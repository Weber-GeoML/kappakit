#!/bin/bash
#SBATCH --job-name 1-dmap-roll                      # Job name
...
python -m kappakit.routines.create_dataset \
    --dataset.name Roll \
    --dataset.save_name "data/roll_N=${num_points}_s=${noise}_diffusion" \
...
python -m kappakit.routines.estimate_curvature \
    --dataset.name "./data/roll_N=${num_points}_s=${noise}_diffusion" \
    --output.name "./results/roll_N=${num_points}_s=${noise}_diffusion" \

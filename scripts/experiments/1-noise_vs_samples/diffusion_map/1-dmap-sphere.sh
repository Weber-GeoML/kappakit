#!/bin/bash
#SBATCH --job-name 1-dmap-sphere                    # Job name
...
python -m kappakit.routines.create_dataset \
    --dataset.name Sphere \
    --dataset.save_name "data/sphere_N=${num_points}_s=${noise}_diffusion" \
...
python -m kappakit.routines.estimate_curvature \
    --dataset.name "./data/sphere_N=${num_points}_s=${noise}_diffusion" \
    --output.name "./results/sphere_N=${num_points}_s=${noise}_diffusion" \

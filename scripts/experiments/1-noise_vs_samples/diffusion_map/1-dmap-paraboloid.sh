#!/bin/bash
#SBATCH --job-name 1-dmap-paraboloid                # Job name
...
python -m kappakit.routines.create_dataset \
    ...
    --dataset.save_name "data/paraboloid_N=${num_points}_s=${noise}_diffusion" \
...
python -m kappakit.routines.estimate_curvature \
    --dataset.name "./data/paraboloid_N=${num_points}_s=${noise}_diffusion" \
    --output.name "./results/paraboloid_N=${num_points}_s=${noise}_diffusion" \

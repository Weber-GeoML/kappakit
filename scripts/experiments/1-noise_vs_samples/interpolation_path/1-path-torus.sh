#!/bin/bash
#SBATCH --job-name 1-path-torus                     # Job name
...
python -m kappakit.routines.create_dataset \
    --dataset.name Torus \
    --dataset.save_name "data/torus_N=${num_points}_s=${noise}_interpolation" \
...
python -m kappakit.routines.train_diffusion_model \
    --dataset.name "./data/torus_N=${num_points}_s=${noise}_interpolation" \
    --dataset.num_points "${num_points}" \
    ...
    --train.num_layers 6 \
    --train.width 1024 \
    --train.save_name "./models/torus_N=${num_points}_s=${noise}_interpolation"
python -m kappakit.routines.estimate_curvature \
    --dataset.name "./data/torus_N=${num_points}_s=${noise}_interpolation" \
    --output.name "./results/torus_N=${num_points}_s=${noise}_interpolation" \
    --dataset.eval_mode all \
    --method.name interpolation \
    --interpolation.model_path "./models/torus_N=${num_points}_s=${noise}_interpolation" \

#!/bin/bash
#SBATCH --job-name 1-path-paraboloid                # Job name
...
python -m kappakit.routines.create_dataset \
    ...
    --dataset.save_name "data/paraboloid_N=${num_points}_s=${noise}_interpolation" \
...
python -m kappakit.routines.train_diffusion_model \
    --dataset.name "./data/paraboloid_N=${num_points}_s=${noise}_interpolation" \
    --dataset.num_points "${num_points}" \
    ...
    --train.num_layers 6 \
    --train.width 1024 \
    --train.save_name "./models/paraboloid_N=${num_points}_s=${noise}_interpolation"
python -m kappakit.routines.estimate_curvature \
    --dataset.name "./data/paraboloid_N=${num_points}_s=${noise}_interpolation" \
    --output.name "./results/paraboloid_N=${num_points}_s=${noise}_interpolation" \
    --method.name interpolation \
    --interpolation.model_path "./models/paraboloid_N=${num_points}_s=${noise}_interpolation" \

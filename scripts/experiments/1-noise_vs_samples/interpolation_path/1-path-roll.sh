#!/bin/bash
#SBATCH --job-name 1-path-roll                      # Job name
...
python -m kappakit.routines.create_dataset \
    --dataset.name Roll \
    --dataset.save_name "data/roll_N=${num_points}_s=${noise}_interpolation" \
...
python -m kappakit.routines.train_diffusion_model \
    --dataset.name "./data/roll_N=${num_points}_s=${noise}_interpolation" \
    --dataset.num_points "${num_points}" \
    ...
    --train.num_layers 6 \
    --train.width 1024 \
    --train.save_name "./models/roll_N=${num_points}_s=${noise}_interpolation"
python -m kappakit.routines.estimate_curvature \
    --dataset.name "./data/roll_N=${num_points}_s=${noise}_interpolation" \
    --output.name "./results/roll_N=${num_points}_s=${noise}_interpolation" \
    --dataset.eval_mode all \
    --method.name interpolation \
    --interpolation.model_path "./models/roll_N=${num_points}_s=${noise}_interpolation" \
    --interpolation.model_type FCN \
    --interpolation.distance 2 \
    --interpolation.num_interpolants 100 \
    --interpolation.diffusion_time 10 \
    --interpolation.intrinsic_dim 2 \
    --interpolation.radius 2 \

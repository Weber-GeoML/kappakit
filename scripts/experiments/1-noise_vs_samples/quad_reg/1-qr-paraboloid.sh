#!/bin/bash
#SBATCH --job-name 1-qr-paraboloid                  # Job name
...
python -m kappakit.routines.create_dataset \
    ...
    --dataset.save_name "data/paraboloid_N=${num_points}_s=${noise}_regression" \
...
python -m kappakit.routines.estimate_curvature \
    --dataset.name "./data/paraboloid_N=${num_points}_s=${noise}_regression" \
    --output.name "./results/paraboloid_N=${num_points}_s=${noise}_regression" \
...
python -m kappakit.routines.create_dataset \
    --dataset.name Paraboloid \
    --dataset.save_name "data/paraboloid_N=${num_points}_s=${noise}_regression_auto" \
    --dataset.num_points ${num_points} \
    --dataset.noise ${noise} \
    --dataset.skip_basis true \
    --paraboloid.intrinsic_dim 2 \
    ...
    --paraboloid.radius 1
python -m kappakit.routines.estimate_curvature \
    --dataset.name "./data/paraboloid_N=${num_points}_s=${noise}_regression_auto" \
    --output.name "./results/paraboloid_N=${num_points}_s=${noise}_regression_auto" \

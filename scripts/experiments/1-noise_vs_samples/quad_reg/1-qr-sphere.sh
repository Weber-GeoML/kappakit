#!/bin/bash
#SBATCH --job-name 1-qr-sphere                      # Job name
...
python -m kappakit.routines.create_dataset \
    --dataset.name Sphere \
    --dataset.save_name "data/sphere_N=${num_points}_s=${noise}_regression" \
...
python -m kappakit.routines.estimate_curvature \
    --dataset.name "./data/sphere_N=${num_points}_s=${noise}_regression" \
    --output.name "./results/sphere_N=${num_points}_s=${noise}_regression" \
...
python -m kappakit.routines.create_dataset \
    --dataset.name Sphere \
    --dataset.save_name "data/sphere_N=${num_points}_s=${noise}_regression_auto" \
    --dataset.num_points ${num_points} \
    --dataset.noise ${noise} \
    --dataset.skip_basis true \
    --sphere.radius 1 \
    ...
    --sphere.ambient_dim 3
python -m kappakit.routines.estimate_curvature \
    --dataset.name "./data/sphere_N=${num_points}_s=${noise}_regression_auto" \
    --output.name "./results/sphere_N=${num_points}_s=${noise}_regression_auto" \

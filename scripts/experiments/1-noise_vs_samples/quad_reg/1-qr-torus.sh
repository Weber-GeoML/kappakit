#!/bin/bash
#SBATCH --job-name 1-qr-torus                       # Job name
...
python -m kappakit.routines.create_dataset \
    --dataset.name Torus \
    --dataset.save_name "data/torus_N=${num_points}_s=${noise}_regression" \
...
python -m kappakit.routines.estimate_curvature \
    --dataset.name "./data/torus_N=${num_points}_s=${noise}_regression" \
    --output.name "./results/torus_N=${num_points}_s=${noise}_regression" \
...
python -m kappakit.routines.create_dataset \
    --dataset.name Torus \
    --dataset.save_name "data/torus_N=${num_points}_s=${noise}_regression_auto" \
    --dataset.num_points ${num_points} \
    --dataset.noise ${noise} \
    --dataset.skip_basis true \
    --torus.major_radius 2 \
    ...
    --torus.ambient_dim 3
python -m kappakit.routines.estimate_curvature \
    --dataset.name "./data/torus_N=${num_points}_s=${noise}_regression_auto" \
    --output.name "./results/torus_N=${num_points}_s=${noise}_regression_auto" \

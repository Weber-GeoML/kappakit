#!/bin/bash
#SBATCH --job-name 1-qr-roll                        # Job name
...
python -m kappakit.routines.create_dataset \
    --dataset.name Roll \
    --dataset.save_name "data/roll_N=${num_points}_s=${noise}_regression" \
...
python -m kappakit.routines.estimate_curvature \
    --dataset.name "./data/roll_N=${num_points}_s=${noise}_regression" \
    --output.name "./results/roll_N=${num_points}_s=${noise}_regression" \
...
python -m kappakit.routines.create_dataset \
    --dataset.name Roll \
    --dataset.save_name "data/roll_N=${num_points}_s=${noise}_regression_auto" \
    --dataset.num_points ${num_points} \
    --dataset.noise ${noise} \
    --dataset.skip_basis true \
    --roll.intrinsic_dim 2 \
    ...
    --roll.width 10
python -m kappakit.routines.estimate_curvature \
    --dataset.name "./data/roll_N=${num_points}_s=${noise}_regression_auto" \
    --output.name "./results/roll_N=${num_points}_s=${noise}_regression_auto" \

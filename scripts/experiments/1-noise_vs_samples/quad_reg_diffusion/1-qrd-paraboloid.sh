#!/bin/bash
#SBATCH --job-name 1-qrd-paraboloid                 # Job name
...
python -m kappakit.routines.create_dataset \
    ...
    --dataset.save_name "data/paraboloid_N=${num_points}_s=${noise}_regression_diffusion" \
    --dataset.num_points ${num_points} \
    --dataset.noise ${noise} \
    --dataset.skip_basis true \
    --paraboloid.intrinsic_dim 2 \
    --paraboloid.ambient_dim 3 \
    --paraboloid.sff "[[1.],[-1.]]" \
    --paraboloid.radius 1
python -m kappakit.routines.train_diffusion_model \
    --dataset.name "./data/paraboloid_N=${num_points}_s=${noise}_regression_diffusion" \
    --dataset.num_points "${num_points}" \
    ...
    --train.num_layers 6 \
    --train.width 1024 \
    --train.save_name "./models/paraboloid_N=${num_points}_s=${noise}_regression_diffusion"
python -m kappakit.routines.estimate_curvature \
    --dataset.name "./data/paraboloid_N=${num_points}_s=${noise}_regression_diffusion" \
    --output.name "./results/paraboloid_N=${num_points}_s=${noise}_regression_diffusion" \
    ...
    --regression.model_path "./models/paraboloid_N=${num_points}_s=${noise}_regression_diffusion" \
...
python -m kappakit.routines.create_dataset \
    --dataset.name Paraboloid \
    --dataset.save_name "data/paraboloid_N=${num_points}_s=${noise}_regression_diffusion_auto" \
    --dataset.num_points ${num_points} \
    --dataset.noise ${noise} \
    --dataset.skip_basis true \
    --paraboloid.intrinsic_dim 2 \
    ...
    --paraboloid.radius 1
python -m kappakit.routines.estimate_curvature \
    --dataset.name "./data/paraboloid_N=${num_points}_s=${noise}_regression_diffusion_auto" \
    --output.name "./results/paraboloid_N=${num_points}_s=${noise}_regression_diffusion_auto" \

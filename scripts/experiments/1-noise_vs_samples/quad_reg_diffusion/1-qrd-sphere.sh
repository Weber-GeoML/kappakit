#!/bin/bash
#SBATCH --job-name 1-qrd-sphere                     # Job name
...
python -m kappakit.routines.create_dataset \
    --dataset.name Sphere \
    --dataset.save_name "data/sphere_N=${num_points}_s=${noise}_regression_diffusion" \
...
python -m kappakit.routines.train_diffusion_model \
    --dataset.name "./data/sphere_N=${num_points}_s=${noise}_regression_diffusion" \
    --dataset.num_points "${num_points}" \
    ...
    --train.num_layers 6 \
    --train.width 1024 \
    --train.save_name "./models/sphere_N=${num_points}_s=${noise}_regression_diffusion"
python -m kappakit.routines.estimate_curvature \
    --dataset.name "./data/sphere_N=${num_points}_s=${noise}_regression_diffusion" \
    --output.name "./results/sphere_N=${num_points}_s=${noise}_regression_diffusion" \
    --dataset.eval_mode all \
    --method.name regression_diffusion \
    --regression.radius 0.4 \
    --regression.pca_radius 0.4 \
    --regression.method numpy \
    --regression.intrinsic_dim 2 \
    --regression.model_path "./models/sphere_N=${num_points}_s=${noise}_regression_diffusion" \
    --regression.model_type FCN \
    --regression.diffusion_distance 0.2 \
    --regression.diffusion_time 100 \
    --regression.num_interpolants 512
python -m kappakit.routines.create_dataset \
    --dataset.name Sphere \
    --dataset.save_name "data/sphere_N=${num_points}_s=${noise}_regression_diffusion_auto" \
    --dataset.num_points ${num_points} \
    --dataset.noise ${noise} \
    --dataset.skip_basis true \
    --sphere.radius 1 \
    ...
    --sphere.ambient_dim 3
python -m kappakit.routines.estimate_curvature \
    --dataset.name "./data/sphere_N=${num_points}_s=${noise}_regression_diffusion_auto" \
    --output.name "./results/sphere_N=${num_points}_s=${noise}_regression_diffusion_auto" \
    --dataset.eval_mode all \

#!/bin/bash
#SBATCH --job-name 1-qrd-torus                      # Job name
...
python -m kappakit.routines.create_dataset \
    --dataset.name Torus \
    --dataset.save_name "data/torus_N=${num_points}_s=${noise}_regression_diffusion" \
...
python -m kappakit.routines.train_diffusion_model \
    --dataset.name "./data/torus_N=${num_points}_s=${noise}_regression_diffusion" \
    --dataset.num_points "${num_points}" \
    ...
    --train.num_layers 6 \
    --train.width 1024 \
    --train.save_name "./models/torus_N=${num_points}_s=${noise}_regression_diffusion"
python -m kappakit.routines.estimate_curvature \
    --dataset.name "./data/torus_N=${num_points}_s=${noise}_regression_diffusion" \
    --output.name "./results/torus_N=${num_points}_s=${noise}_regression_diffusion" \
    ...
    --regression.model_path "./models/torus_N=${num_points}_s=${noise}_regression_diffusion" \
...
python -m kappakit.routines.create_dataset \
    --dataset.name Torus \
    --dataset.save_name "data/torus_N=${num_points}_s=${noise}_regression_diffusion_auto" \
    --dataset.num_points ${num_points} \
    --dataset.noise ${noise} \
    --dataset.skip_basis true \
    --torus.major_radius 2 \
    ...
    --torus.ambient_dim 3
python -m kappakit.routines.estimate_curvature \
    --dataset.name "./data/torus_N=${num_points}_s=${noise}_regression_diffusion_auto" \
    --output.name "./results/torus_N=${num_points}_s=${noise}_regression_diffusion_auto" \
    --dataset.eval_mode all \

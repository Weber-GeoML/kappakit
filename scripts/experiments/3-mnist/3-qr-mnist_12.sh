#!/bin/bash
#SBATCH --job-name 3-qr-mnist-12                    # Job name
...
python -m kappakit.routines.upsample_with_diffusion \
    --upsample.dataset ./data/mnist_12 \
    --upsample.model ./models/mnist_12 \
    ...
    --upsample.save_name "./data/mnist_12_x0=${index}"
python -m kappakit.routines.estimate_curvature \
    --dataset.name "./data/mnist_12_x0=${index}" \
    --output.name "./results/mnist_12_x0=${index}_regression" \
    --dataset.eval_mode 1 \

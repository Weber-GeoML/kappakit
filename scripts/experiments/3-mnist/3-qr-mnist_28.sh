#!/bin/bash
#SBATCH --job-name 3-qr-mnist-28                    # Job name
...
python -m kappakit.routines.upsample_with_diffusion \
    --upsample.dataset ./data/mnist_28 \
    --upsample.model ./models/mnist_28 \
    ...
    --upsample.save_name "./data/mnist_28_x0=${index}"
python -m kappakit.routines.estimate_curvature \
    --dataset.name "./data/mnist_28_x0=${index}" \
    --output.name "./results/mnist_28_x0=${index}_regression" \
    --dataset.eval_mode 1 \

#!/bin/bash
#SBATCH --job-name 3-qr-fmnist-28                   # Job name
...
python -m kappakit.routines.upsample_with_diffusion \
    --upsample.dataset ./data/fmnist_28 \
    --upsample.model ./models/fmnist_28 \
    ...
    --upsample.save_name "./data/fmnist_28_x0=${index}"
python -m kappakit.routines.estimate_curvature \
    --dataset.name "./data/fmnist_28_x0=${index}" \
    --output.name "./results/fmnist_28_x0=${index}_regression" \
    --dataset.eval_mode 1 \

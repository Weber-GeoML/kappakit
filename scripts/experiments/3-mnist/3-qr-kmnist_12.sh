#!/bin/bash
#SBATCH --job-name 3-qr-kmnist-12                   # Job name
...
python -m kappakit.routines.upsample_with_diffusion \
    --upsample.dataset ./data/kmnist_12 \
    --upsample.model ./models/kmnist_12 \
    ...
    --upsample.save_name "./data/kmnist_12_x0=${index}"
python -m kappakit.routines.estimate_curvature \
    --dataset.name "./data/kmnist_12_x0=${index}" \
    --output.name "./results/kmnist_12_x0=${index}_regression" \
    --dataset.eval_mode 1 \

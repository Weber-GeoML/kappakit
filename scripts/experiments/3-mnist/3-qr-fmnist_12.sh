#!/bin/bash
#SBATCH --job-name 3-qr-fmnist-12                   # Job name
...
python -m kappakit.routines.upsample_with_diffusion \
    --upsample.dataset ./data/fmnist_12 \
    --upsample.model ./models/fmnist_12 \
    ...
    --upsample.save_name "./data/fmnist_12_x0=${index}"
python -m kappakit.routines.estimate_curvature \
    --dataset.name "./data/fmnist_12_x0=${index}" \
    --output.name "./results/fmnist_12_x0=${index}_regression" \
    --dataset.eval_mode 1 \

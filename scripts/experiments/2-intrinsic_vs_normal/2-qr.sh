...
python -m kappakit.routines.create_dataset \
    ...
    --dataset.save_name "data/paraboloid_d=${intrinsic_dim}_n=${normal_dim}_regression" \
...
python -m kappakit.routines.estimate_curvature \
    --dataset.name "./data/paraboloid_d=${intrinsic_dim}_n=${normal_dim}_regression" \
    --output.name "./results/paraboloid_d=${intrinsic_dim}_n=${normal_dim}_regression" \
    --method.name regression \
    --regression.radius 1 \
    --regression.pca_radius 1 \
    --regression.method numpy \
    --regression.intrinsic_dim ${intrinsic_dim}

python -m kappakit.routines.create_dataset \
    --dataset.name Paraboloid \
    --dataset.save_name "data/paraboloid_d=${intrinsic_dim}_n=${normal_dim}_regression_auto" \
    --dataset.num_points 5000 \
    --dataset.noise 0 \
    --dataset.skip_basis true \
    --paraboloid.intrinsic_dim ${intrinsic_dim} \
    ...
    --paraboloid.radius 1
python -m kappakit.routines.estimate_curvature \
    --dataset.name "./data/paraboloid_d=${intrinsic_dim}_n=${normal_dim}_regression_auto" \
    --output.name "./results/paraboloid_d=${intrinsic_dim}_n=${normal_dim}_regression_auto" \

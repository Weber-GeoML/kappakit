#!/bin/bash
#SBATCH --job-name 2-qrd                            # Job name
...
python -m kappakit.routines.create_dataset \
    ...
    --dataset.save_name "data/paraboloid_d=${intrinsic_dim}_n=${normal_dim}_regression_diffusion" \
...
python -m kappakit.routines.train_diffusion_model \
    --dataset.name "./data/paraboloid_d=${intrinsic_dim}_n=${normal_dim}_regression_diffusion" \
    --dataset.num_points 5000 \
    ...
    --train.num_layers 6 \
    --train.width 1024 \
    --train.save_name "./models/paraboloid_d=${intrinsic_dim}_n=${normal_dim}_regression_diffusion"
python -m kappakit.routines.estimate_curvature \
    --dataset.name "./data/paraboloid_d=${intrinsic_dim}_n=${normal_dim}_regression_diffusion" \
    --output.name "./results/paraboloid_d=${intrinsic_dim}_n=${normal_dim}_regression_diffusion" \
    --method.name regression_diffusion \
    --regression.radius 0.5 \
    --regression.pca_radius 0.5 \
    --regression.method numpy \
    --regression.intrinsic_dim 2 \
    --regression.model_path "./models/paraboloid_d=${intrinsic_dim}_n=${normal_dim}_regression_diffusion" \
    --regression.model_type FCN \
    --regression.diffusion_distance 0.3 \
    --regression.diffusion_time 100 \
    --regression.num_interpolants 512
python -m kappakit.routines.create_dataset \
    --dataset.name Paraboloid \
    --dataset.save_name "data/paraboloid_d=${intrinsic_dim}_n=${normal_dim}_regression_diffusion_auto" \
    --dataset.num_points 5000 \
    --dataset.noise 0 \
    --dataset.skip_basis true \
    --paraboloid.intrinsic_dim ${intrinsic_dim} \
    ...
    --paraboloid.radius 1
python -m kappakit.routines.estimate_curvature \
    --dataset.name "./data/paraboloid_d=${intrinsic_dim}_n=${normal_dim}_regression_diffusion_auto" \
    --output.name "./results/paraboloid_d=${intrinsic_dim}_n=${normal_dim}_regression_diffusion_auto" \
    --method.name regression_diffusion_auto \
    --regression.pca_batch_size 10 \
    --regression.reg_batch_size 10 \
    --regression.intrinsic_dim 2 \
    --regression.model_path "./models/paraboloid_d=${intrinsic_dim}_n=${normal_dim}_regression_diffusion" \
    --regression.model_type FCN \
    --regression.diffusion_distance 0.3 \
